"""
Professional 3D Dashboard for IR-01 Energy-Aware Robot Grid Exploration
"""
...
class GridSim:
    ...
    def generate(self):
        ...
        while True:
            g = (rng.random((self.size,self.size)) < self.prob).astype(int); g[0,0] = 0
            r = self._reach(g); free = int(np.sum(g==0)); rf = int(np.sum(r & (g==0)))
            if free > 0 and rf/free >= 0.70: self.grid = g; self.total_reachable = rf; return
    def _reach(self, g):
...
def run_sim(params):
    ...
    gs.generate(); sz = gs.size; energy = params.get('energy',400); sensing = params.get('sensing',2)
    speed = params.get('speed',0.05); pos = (0,0); revealed = {}; traj = [pos]
    visited = {pos}
    dirs = [(-1,0),(1,0),(0,-1),(0,1)]; dn = {(-1,0):'UP',(1,0):'DOWN',(0,-1):'LEFT',(0,1):'RIGHT'}
        ...
        for dx in range(-sensing, sensing+1):
            for dy in range(-sensing, sensing+1):
                if abs(dx)+abs(dy) <= sensing:
                    nx,ny = p[0]+dx, p[1]+dy
                    if ib(nx,ny) and (nx,ny) not in revealed:
                        revealed[(nx,ny)] = int(gs.grid[nx,ny])
    ...
    def get_frontiers():
        ...
        for p, occ in revealed.items():
            ...
            for dx,dy in dirs:
                if ib(p[0]+dx, p[1]+dy) and (p[0]+dx, p[1]+dy) not in revealed:
                    fr.add(p); break
        return list(fr)
    ...
    def info_gain(p):
        ...
        for dx in range(-2, 3):
            for dy in range(-2, 3):
                if abs(dx)+abs(dy) <= 2 and ib(p[0]+dx, p[1]+dy) and (p[0]+dx, p[1]+dy) not in revealed:
                    c += 1
        return c
    ...
    for step in range(sz*sz*4):
        ...
        if energy <= dist_to_base + 15:
            ...
            for f in frontiers:
                ...
                ig = info_gain(f)
                score = ig / max(1, d_to_f)
                if f not in visited: score *= 1.2
                if score > best_score:
        ...
        if ib(np_[0], np_[1]) and revealed.get(np_) != 1:
            pos = np_; energy -= 1; traj.append(pos); visited.add(pos); sense(pos)
        else:
            energy -= 1
        fr_c = sum(1 for v in revealed.values() if v == 0)
        cov = min(1.0, fr_c/gs.total_reachable) if gs.total_reachable > 0 else 0
        sim_state.update(revealed={f"{k[0]},{k[1]}":v for k,v in revealed.items()},
            position=list(pos), trajectory=[list(t) for t in traj], coverage=cov, step=step+1, energy=energy)
        time.sleep(speed)
...
@app.route('/')
def index(): return render_template('dashboard3d.html')
...
if __name__ == '__main__':
    ...
    print("3D Dashboard: http://localhost:5001")
